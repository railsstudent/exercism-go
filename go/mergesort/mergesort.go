package main

import "fmt"
import "sync"
//import "time"

func Merge(ldata []int, rdata []int) (result []int) {
    result = make([]int, len(ldata) + len(rdata))
    lidx, ridx := 0, 0

    for i:=0;i<cap(result);i++ {
        switch {
            case lidx >= len(ldata):
                result[i] = rdata[ridx]
                ridx++
            case ridx >= len(rdata):
                result[i] = ldata[lidx]
                lidx++
            case ldata[lidx] < rdata[ridx]:
                result[i] = ldata[lidx]
                lidx++
            default:
                result[i] = rdata[ridx]
                ridx++
        }
    }

    return
}

func MergeSort(data []int, r chan []int) {
    if len(data) == 1 {
        r <- data
        return
    }

    leftChan := make(chan []int)
    rightChan := make(chan []int)
    middle := len(data)/2

    go MergeSort(data[:middle], leftChan)
    go MergeSort(data[middle:], rightChan)

    ldata := <-leftChan
    rdata := <-rightChan

    close(leftChan)
    close(rightChan)
    r <- Merge(ldata, rdata)
    return
}

func main() {
    s := []int{22, 8, 3, 31, 4, 2, 42, 1, 16, 6, 11, 25, 9, 8, 10, 12, 18, 14, 7, 15}

    arrs := [][]int{}
    arrs = append(arrs, s);
    arrs = append(arrs, []int{19, 17, 15, 11, 13, 9, 1, 3, 7, 5, 77, 66, 22, 33, 99, 88, 55, 44, 89, 87});
    arrs = append(arrs, []int{10, 8, 6, 0, 2, 4});
    arrs = append(arrs, []int{9, 7, 5, 1, 3});

    var wg sync.WaitGroup
    //wg.Add(len(arrs))

    result := make(chan []int, len(arrs))
    for i := range arrs {
      wg.Add(1)
      go func(i int) {
        defer wg.Done()
        MergeSort(arrs[i], result)
      } (i)
    }

    go func() {
      wg.Wait()
      close(result)
    } ()

    for r := range result {
      for _, v := range r {
         fmt.Printf("%d, ", v)
      }
      fmt.Println()
    }
    fmt.Println("main finished")
}
